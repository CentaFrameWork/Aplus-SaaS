...
@interface AppDelegate ()<AdLoadingDelegate,AlertInputPwdViewDelegate,ResponseDelegate,UIAlertViewDelegate,BMKGeneralDelegate, BMKLocationAuthDelegate>
{
    UIWindow *_adLondingWindow;
    UIWindow *_mainWindow;
    UIViewController *jumpBaseVC;
    FaceRecogVC *_faceRecogVC;
    UIView *_shadowView;
...
- (instancetype)init
{
    ...
    if (self)
    {
        ...
        {
            //  处理天津人脸识别
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(inputDismissNotification:) name:@"HaveDismiss" object:nil];
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(changeVerifyType:) name:ChangeVerifyType object:nil];
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(faceRecogSuccess) name:FaceRecogSuccess object:nil];
        }
    }
    return self;
}
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    // 检查用户版本，存储首次安装的配置(要在上面的代码之后调用，不然上面的代码永远不执行)
...
- (void)checkFaceSwitch
{
    ...
    if (isLoginSuccess)
    {
        int tips = [[CommonMethod getUserdefaultWithKey:@"FaceTips"] intValue];
        if (tips == 1)
        {
            // 如果已采集人脸，且“我的”开关关闭，弹框提示
            NSString *existUrl = [CommonMethod getUserdefaultWithKey:FaceCollectUrl];
            BOOL isOn = [[CommonMethod getUserdefaultWithKey:FaceRecogSwitch] boolValue];
            float f = [[CommonMethod getUserdefaultWithKey:@"FaceOption"] floatValue];
            
            if ([existUrl contains:@"http"] && !isOn && f>0)
            {
                NSString *tipStr = @"您已采集了人脸数据，小伙伴们请前往“我的”-人脸识别，开启此功能~";
                UIAlertView *tipAlert = [[UIAlertView alloc] initWithTitle:nil
                                                                   message:tipStr
                                                                  delegate:self
                                                         cancelButtonTitle:nil
                                                         otherButtonTitles:@"去设置", @"忽略", nil];
                tipAlert.tag = FaceRecogAlertTag;
                [tipAlert show];
            }
            else if (![existUrl contains:@"http"])
            {
                NSString *tipStr = @"本次升级人脸识别功能，小伙伴们请前往”我的“-人脸采集，进行人脸数据采集~";
                UIAlertView *tipAlert = [[UIAlertView alloc] initWithTitle:nil
                                                                   message:tipStr
                                                                  delegate:self
                                                         cancelButtonTitle:nil
...
- (void)addAdLondingView
{
    ...
    _adLondingWindow = [[UIWindow alloc]initWithFrame:screenBounds];
    UIImageView *bgImageView = [[UIImageView alloc]initWithFrame:_adLondingWindow.bounds];
    
    UIImage *logoImage = [UIImage imageNamed:@"ZFLogo.png"];
    UIImageView *logoImageView = [[UIImageView alloc] initWithFrame:CGRectMake(screenBounds.size.width/2-logoImage.size.width/2,
                                                                               80,
                                                                               logoImage.size.width,
                                                                               logoImage.size.height)];
    [logoImageView setImage:logoImage];
    
    [_adLondingWindow addSubview:bgImageView];
    [_adLondingWindow addSubview:logoImageView];
    
    _adLoadingViewController = [[AdLoadingViewController alloc]initWithNibName:@"AdLoadingViewController"
                                                                        bundle:nil];
    _adLoadingViewController.delegate = self;
    _adLondingWindow.rootViewController = _adLoadingViewController;
...
- (void)validGestureView
{
    ...
    else
    {
        ...
        if (_showFaceRecog && isOn && [CityCodeVersion isTianJin])
        {
            // 人脸识别总开关开启
            _faceRecogVC = [[FaceRecogVC alloc] initWithDefauleSetting:NO];
            // 人脸识别
...
- (void)launchInputPwd
{
    ...
    AlertInputPwdView *customView = [array firstObject];
    customView.delegate = self;
    [customView showClose:NO];
    _isPwdShow = YES;

    customView.frame = CGRectMake(0, 0, APP_SCREEN_WIDTH, APP_SCREEN_HEIGHT);
    
    [_adLondingWindow addSubview:customView];
}
...
/// 人脸识别
- (void)showFaceRecogVC
{
    if (!_shadowView)
    {
        _shadowView = [[UIView alloc] initWithFrame:[UIScreen mainScreen].bounds];
        _shadowView.backgroundColor = [UIColor colorWithRed:1 green:1 blue:1 alpha:0.95];
        [self.window.rootViewController.view addSubview:_shadowView];
    }
   
    CustomPanNavgationController *navgationVC = [[CustomPanNavgationController alloc] initWithRootViewController:_faceRecogVC];

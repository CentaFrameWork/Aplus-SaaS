...
#import "DecodePhoneNoUtils.h"
#import <CommonCrypto/CommonCryptor.h>
#import "GTMBase64.h"
#import "NSString+MD5Additions.h"

static DecodePhoneNoUtils *sharedDecodeUtils;

@implementation DecodePhoneNoUtils

+ (DecodePhoneNoUtils *)sharedDecodeUtils
{
    if (!sharedDecodeUtils)
    {
        sharedDecodeUtils = [[self alloc] init];
    }
    
    return sharedDecodeUtils;
}

- (NSString *)decodePhoneNo:(NSString *)encodeNo
{
    NSString *decodeStr = @"";
    ...
    //时间戳
    NSString *dateStr = [CommonMethod getSysDateStrWithFormat:@"yyMMdd"];
    NSInteger dateNum = [dateStr integerValue];
    NSInteger number = (dateNum*5+177)*2-33;
    NSString *unixTimeStr = [NSString stringWithFormat:@"%@",@(number)];
    
    NSLog(@"unixTimeStr:%@",unixTimeStr);
    
    NSString *osign = [NSString stringWithFormat:@"%@%@%@",key,company,unixTimeStr];
    NSString *sign = [osign md5];
    
    NSString *encodeKey = [sign substringToIndex:8];

    decodeStr = [self encryptWithContent:encodeNo type:kCCDecrypt key:encodeKey];

...
+ (SQLiteManager *)shareSQLiteManagerWithDatabasePath:(NSString *)databasePath{
    ...
    [[self shareSQLiteManager] closeDatabase];
    
    [self shareSQLiteManager].db = [FMDatabase databaseWithPath:databasePath];
    
    [self shareSQLiteManager].databasePath = databasePath;
...
+ (BOOL)insertToTableWithDataArray:(NSArray<NSObject *> *)dataArray andTableName:(NSString *)tableName{
    ...
    tableName = tableName ? : NSStringFromClass([[dataArray firstObject] class]);
    
    @try {
        for (NSObject * object in dataArray) {
            
            BOOL b = [SQLiteManager insertOrReplaceByObject:object andTableName:tableName andDB:db];
...
+ (id)queryOneWithTableName:(NSString *)tableName andClass:(Class)clazz{
    ...
    NSMutableArray * array = [self queryAllWithTableName:tableName andClass:clazz];
    ...
    while (rs.next) {
        ...
        for (NSString * prop in propertyArray) {
            if(![rs columnIsNull:prop]){
                id value = [rs objectForColumn:prop];
...
+ (BOOL)deleteToTableWithSQL:(NSString *)sql andParams:(NSArray<NSArray *> *)params{
    ...
    @try {
        for (NSArray * paramArray in params) {
            BOOL b = [db executeUpdate:sql withArgumentsInArray:paramArray];
            if (b==false) {
                res=false;
                NSLog(@"删除失败！");
...
+ (BOOL)insertOrReplaceByObject:(id)object andTableName:(NSString *)tableName andDB:(FMDatabase *)db{
    
    NSMutableArray * valueArray=[[NSMutableArray alloc]init];
    
    NSArray * propertyArray=[SQLiteManager propertyArrayWithClass:[object class]];
    NSMutableString * str=[[NSMutableString alloc]init];
    NSMutableString * sql=[NSMutableString stringWithFormat:@"replace into %@(",tableName==nil?NSStringFromClass([object class]):tableName];
    BOOL b = false;
    for (int i=0;i<propertyArray.count;i++) {
    ...
    return [db executeUpdate:sql withArgumentsInArray:valueArray];
}
...
+ (NSMutableArray *)propertyArrayWithClass:(Class)clazz{
    ...
    NSArray * ignoredPropertyNames = [clazz respondsToSelector:@selector(chh_cache_ignoredPropertyNames)] ? [clazz chh_cache_ignoredPropertyNames] : nil;
    
    NSArray * allowedPropertyNames = [clazz respondsToSelector:@selector(chh_cache_allowedPropertyNames)] ? [clazz chh_cache_allowedPropertyNames] : nil;
    
    if (allowedPropertyNames.count > 0) {
        
        NSMutableArray * tmpArr = [NSMutableArray arrayWithArray:allowedPropertyNames];
        
        [tmpArr removeObjectsInArray:ignoredPropertyNames];
        
        [properties addObjectsFromArray:tmpArr];
        
    }else{
        
        [clazz mj_enumerateProperties:^(MJProperty *property, BOOL *stop) {
            
            for (NSString * ignoredPropName in ignoredPropertyNames) {
                
                if ([ignoredPropName isEqualToString:property.name]) {
...
+ (void)checkAndUpdateDatabaseVersion{
    ...
    manager.appVersion = [infoDictionary objectForKey:@"CFBundleShortVersionString"];
    //设置数据库文件地址
    ...
    //获取本地文件，取得数据库版本
    manager.databasePath = databasePath;
    NSString * sql = [NSString stringWithFormat:@"select * from %@;", DATABASE_SETTING_TABLE_NAME];
...
- (void)updateDatabase{
    ...
    SettingSystem * settingSystem = [SettingSystem settingSystemForDatabase] ? : [[SettingSystem alloc] init];
    
    if(self.databaseVersion == nil || [self.databaseVersion isEqualToString:VERSION_0]){
        
        NSLog(@"进行首次安装");
        
        settingSystem.database_version = VERSION_1_0_0;
        
    }
    
//    if ([self.databaseVersion isEqualToString:VERSION_2_0_0]) {
//
//        NSLog(@"进行2.0.0->2.1.1升级");
//
//        [self DMLSQL:VERSION_SQL_2_1_1_1 andArgumentsInArray:nil];
//
//        self.databaseVersion = VERSION_2_1_1;
//    }
    
    BOOL res = [settingSystem saveSettingSystemToDatabase];
    
    NSLog(@"-------->数据库版本数据保存%@", res ? @"成功" : @"失败");
}

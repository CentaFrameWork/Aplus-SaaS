...
#define kRowHeight 42.0f
#define kRowLineHeight 0.5f
#define kSeparatorHeight 5.0f

#define kTitleFontSize 10.0f
#define kButtonTitleFontSize 14.0f
...
- (instancetype)initWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle destructiveButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSArray *)otherButtonTitles handler:(ALActionSheetViewDidSelectButtonBlock)block;
{
    ...
    if (self)
    {
        ...
        if (_title && _title.length>0)
        {
            ...
            UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, width, titleHeight+4)];
            ...
            offy += titleHeight+kRowLineHeight+4;
        }
        ...
        if ([_otherButtonTitles count] > 0)
        {
            for (int i = 0; i < _otherButtonTitles.count; i++)
            {
                ...
                [btn setTitleColor:[UIColor colorWithRed:224.0f/255.0f green:20.0f/255.0f blue:15.0f/255.0f alpha:1.0f] forState:UIControlStateNormal];
        ...
        _actionSheetHeight = offy+BOTTOM_SAFE_HEIGHT;

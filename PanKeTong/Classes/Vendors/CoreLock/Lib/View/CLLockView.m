...
-(void)drawRect:(CGRect)rect{
    ...
    [self drawCircleWithRect:rect andCtx:ctx];
    
    [self drawLineWithRect:rect andCtx:ctx];
    
    
}

- (void)drawCircleWithRect:(CGRect)rect andCtx:(CGContextRef)ctx{
    ...
    [_itemViewsM enumerateObjectsUsingBlock:^(CLLockItemView *itemView, NSUInteger idx, BOOL *stop) {
        ...
        CGFloat sizeWH = itemView.bounds.size.width - lineW;
        CGFloat originXY = lineW *.5f;
        
        //添加一个圆环路径
        CGRect frame = (CGRect){CGPointMake(itemView.x + originXY, itemView.y + originXY), CGSizeMake(sizeWH, sizeWH)};
        
        //添加一个圆环路径
        CGPathAddEllipseInRect(loopPath, NULL, frame);
...
- (void)drawLineWithRect:(CGRect)rect andCtx:(CGContextRef)ctx{
    ...
    //设置线宽
    CGContextSetLineWidth(ctx, 8*NewRatio);
...
-(void)layoutSubviews{
    ...
    CGFloat topSpace = 14*NewRatio;//距离父View上空隙
    
    CGFloat leftSpace = 0;//距离父View左空隙
    
    CGFloat hSpace = 55*NewRatio;//子view横向间隔
    
    CGFloat zSpace = 48*NewRatio;//子view纵向间隔
    
    CGFloat itemViewWH = (self.width - (2 * leftSpace) - (rowCount - 1) * hSpace)/rowCount;
...
//        CGFloat y =marginValue * (col +1) + col * itemViewWH;
        ...
        CGFloat x = leftSpace + row * (hSpace + itemViewWH);
        
        CGFloat y = topSpace + col * (zSpace + itemViewWH);

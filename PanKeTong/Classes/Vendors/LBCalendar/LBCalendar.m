...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    [self performSelector:@selector(starNetwork) withObject:nil afterDelay:1];
}

- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
//    [self updatePage];
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(starNetwork) object:nil];
}

- (void)starNetwork{
    [self.myDelegate getEventDataWithDate:_currentDate];
    NSLog(@"-------------网络请求－－－－－－－－－－－－－");
}

...
#import "ChannelCallBJPresenter.h"
#import "ChannelCallNJPresenter.h"
#import "ChannelCallTJPresenter.h"
#import "ChannelCallSZPresenter.h"
#import "ChannelCallCQPresenter.h"
#import "ChannelCallGZPresenter.h"
#import "ChannelCallAMPresenter.h"
#import "ChannelCallCSPresenter.h"


@interface ChannelCallViewController () <BJTurnPrivateDelegate,ChannelCallViewProtocol>
{
    ...
    ChannelCallBasePresenter *_channelCallPresenter;
}

@end

@implementation ChannelCallViewController
...
- (void)initPresenter
{
    if ([CityCodeVersion isChongQing]) {
        _channelCallPresenter = [[ChannelCallCQPresenter alloc] initWithDelegate:self];
    }
    else if ([CityCodeVersion isShenZhen]) {
        _channelCallPresenter = [[ChannelCallSZPresenter alloc] initWithDelegate:self];
    }
    else if ([CityCodeVersion isGuangZhou]) {
        _channelCallPresenter = [[ChannelCallGZPresenter alloc] initWithDelegate:self];
    }
    else if ([CityCodeVersion isAoMenHengQin]) {
        _channelCallPresenter = [[ChannelCallAMPresenter alloc] initWithDelegate:self];
    }
    else if ([CityCodeVersion isBeiJing]) {
        _channelCallPresenter = [[ChannelCallBJPresenter alloc] initWithDelegate:self];
    }
    else if ([CityCodeVersion isNanJing]) {
        _channelCallPresenter = [[ChannelCallNJPresenter alloc] initWithDelegate:self];
    }
    else if ([CityCodeVersion isTianJin]) {
        _channelCallPresenter = [[ChannelCallTJPresenter alloc] initWithDelegate:self];
    }
    else if ([CityCodeVersion isChangSha]) {
        _channelCallPresenter = [[ChannelCallCSPresenter alloc] initWithDelegate:self];
    }else{
        _channelCallPresenter = [[ChannelCallBasePresenter alloc] initWithDelegate:self];
    }
}
...
- (void)goTurnPrivateCustomer
{
    ...
    // 是否座机手机有区分
    if ([_channelCallPresenter haveDistinguishLandlineAndMobile])
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ChannelCallModelEntity *channelCall = _tableViewSource[row];
    
    NSString *propInfo = @" ";
    if(channelCall.motherNetComments && ![channelCall.motherNetComments isEqualToString:@""])
    {
        propInfo = channelCall.motherNetComments;
    }
    NSString *channelSource = @" ";
    if(channelCall.mchannelInquirySources && ![channelCall.mchannelInquirySources isEqualToString:@""])
    {
        channelSource = channelCall.mchannelInquirySources;
    }

    double timeDouble = [CommonMethod tryTimeNumberWith:channelCall.mcreateTime];
    NSString *time = [CommonMethod dateConcretelyTime:timeDouble andYearNum:[[channelCall.mcreateTime substringToIndex:4] integerValue]];
    ...
    channelCallTableViewCell.channelPhone.text = channelCall.mphone;
    channelCallTableViewCell.phoneAddress.text = address;
    channelCallTableViewCell.agentName.text = channelCall.mchannelInquiryChief;
    channelCallTableViewCell.propInfo.text = propInfo;
    channelCallTableViewCell.channelSource.text = channelSource;
    channelCallTableViewCell.createTime.text = time;

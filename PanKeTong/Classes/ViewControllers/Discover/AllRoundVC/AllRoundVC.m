...
#define TagImageWidth                   (APP_SCREEN_WIDTH / 3)- ((APP_SCREEN_WIDTH / 3) / 2- (size.width/2)) + 3

@interface AllRoundVC ()<UITableViewDataSource,UITableViewDelegate,ButtonClickDelegate,
FilterViewDelegate,FilterListDelegate,MoreFilterInfoDelegate,AllRoundFilterCustomViewDelegate,
SearchResultDelegate,UITextFieldDelegate,EditRefreshDelegate,AllRoundListViewProtocol>
{
...
- (void)initArray
{
    ...
    if (self.isPropList && [CityCodeVersion isDongGuan])
    {
        // 通盘房源
        NSMutableArray *array = [NSMutableArray arrayWithArray:_tagTextArray];
        int count = (int)array.count;
        [array insertObject:@"实勘" atIndex:count-1];
        [array insertObject:@"钥匙" atIndex:count-1];
        _tagTextArray = array;
    }
    // 售价数组
...
- (void)initView
{
    ...
    // 创建搜索框
    [self createTopSearchBarViewWithTextField:_searchTextfield andRightBtn:_rightSearchBtn];
...
//        if (![NSString isNilOrEmpty:_houseNo])
//        {
//            _searchKeyWord = [NSString stringWithFormat:@"%@，%@",_searchKeyWord,_houseNo];
//        }
    }
...
- (void)firstTitle:(NSNotification *)text
{
    ...
    imageView.frame = CGRectMake((APP_SCREEN_WIDTH / 3) - ((APP_SCREEN_WIDTH / 3) / 2 - (size.width / 2)) + 3,
                                 label.center.y - 2,
                                 8,
                                 6);
...
- (void)secondTitle:(NSNotification *)text
{
    ...
    NSDictionary *attribute = @{NSFontAttributeName:[UIFont fontWithName:FontName  size:14.0]};
    CGSize size = [value boundingRectWithSize:CGSizeMake(MAXFLOAT, 28) options: NSStringDrawingTruncatesLastVisibleLine attributes:attribute context:nil].size;
    imageView.frame = CGRectMake(TagImageWidth, label.center.y-2, 8, 6);
}
...
- (void)sortAction
{
    ...
    NSArray * listArr = @[@"售价从大到小", @"售价从小到大", @"租价从大到小", @"租价从小到大", @"恢复默认排序",];
    
    __block typeof(self) weakSelf = self;
    
    SelectorView * selectView = [NewUtils popoverSelectorTitle:nil listArray:listArr theOption:^(NSInteger optionValue) {
        
        [weakSelf selectSortActionWithIndex:optionValue];
        
    }];
    
    NSInteger index = [listArr indexOfObject:_currentSort];
    
    [selectView setSelectIndex:index];
...
//    {
//        _searchTextfield.text = [NSString stringWithFormat:@"%@,%@",_searchKeyWord,_houseNo];
//    }
//    else
//    {
...
- (void)editRefresh:(PropPageDetailEntity *)entity
{
    ...
    if ([_allRoundListPresenter haveEditFunction])
    {
        if (_selectRow >= 0)
        {
            ...
            oldEntity.square = entity.squareEdit ? entity.squareEdit : entity.square;
...
- (void)clickHeadImageMethod:(UIButton *)button
{
    ...
    if (realSurveyCount.integerValue > 0)
    {
        ...
        {
            // 广州
            if (_allRoundListPresenter.isHaveUploadVideo)
            {

                SelectRealVC *selectRealVC = [[SelectRealVC alloc] initWithNibName:@"SelectRealVC" bundle:nil];
                selectRealVC.propKeyId = propModelEntity.keyId;
                selectRealVC.isItem = NO;
                [self.navigationController pushViewController:selectRealVC animated:YES];
            }
            else
            {
...
- (void)getFilterEntity:(FilterEntity *)entity
{
    ...
    NSString * maxString;
    NSString * typeString;
    if ([NSString isNilOrEmpty:_filterEntity.moreFilterMinSalePrice]
        && [NSString isNilOrEmpty:_filterEntity.moreFilterMaxSalePrice])
    {
    ...
    {
        if (_filterEntity.minSalePrice
            && _filterEntity.maxSalePrice
            && ![_filterEntity.minSalePrice isEqualToString:@""]
            && ![_filterEntity.maxSalePrice isEqualToString:@""])
        {
            price = [NSString stringWithFormat:@"%@-%@万元",_filterEntity.minSalePrice,_filterEntity.maxSalePrice];
        }
        else if (_filterEntity.minRentPrice
                 && _filterEntity.maxRentPrice
                 && ![_filterEntity.minRentPrice isEqualToString:@""]
                 && ![_filterEntity.maxRentPrice isEqualToString:@""])
        {
...
- (void)searchResultWithKeyword:(NSString *)keyword
                  andExtendAttr:(NSString *)extendAttr
                   andItemValue:(NSString *)itemvalue
...
- (void)headerRefreshMethod
{
    ...
    _filterEntity.isVideo = @"false";
    for (SelectItemDtoEntity * itemDtoEntity in _filterEntity.propTag)
    {
        if ([itemDtoEntity.itemText isEqualToString:@"视频房源"])
        {
            _filterEntity.isVideo = @"true";
        }
    }
    ...
    _propListApi.isVideo = _filterEntity.isVideo;
...
//    [AFUtils POST:AipPropertyWarZone parameters:[_propListApi getBody] controller:self Dict:^(NSDictionary *statusDict) {
//        
//    } failure:^(NSError *statusError) {
//        
//    }];
...
- (void)getTopViewFilterCondition
{
    ...
    secondImageView.frame = CGRectMake(TagImageWidth, label.center.y-2, 8, 6);
    ...
    thirdImageView.frame = CGRectMake(TagImageWidth, label.center.y-2, 8, 6);
}
...
- (NSString *)getFilterConditionString
{
    ...
    {
        [filterArray addObject:[NSString stringWithFormat:@"房屋等级：%@",[self isHaveString:_filterEntity.roomLevelText]]];
    }
...
- (NSMutableArray*)getfilterConditionWith:(NSMutableArray *)array
{
    ...
    {
        SelectItemDtoEntity * entity = array[i];
        if (![entity.itemText isEqualToString:@"视频房源"])
        {
            if (entity.itemValue)
            {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_propListArray.count != 0)
    {
        ...
        {
            if ([propModelEntity.isVideoPhotoAddress integerValue] == 1)
            {
        ...
        if (propModelEntity.trustType.integerValue == SALE ||
            propModelEntity.trustType.integerValue == BOTH)
        {
        ...
        if (propModelEntity.propertyTags.count == SALE)
        {
        ...
        else if (propModelEntity.propertyTags.count == RENT)
        {
            PropertyTagEntity *firstTagEntity = [propModelEntity.propertyTags objectAtIndex:0];
        ...
        else if (propModelEntity.propertyTags.count >= BOTH)
        {
            PropertyTagEntity *firstTagEntity = [propModelEntity.propertyTags objectAtIndex:0];
            PropertyTagEntity *secondTagEntity = [propModelEntity.propertyTags objectAtIndex:1];
...
#pragma mark - private
- (void)selectSortActionWithIndex:(NSInteger)index{
